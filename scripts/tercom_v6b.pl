...
if ($OUTPUT_TYPES{"sum"}) {
    ...
    print "Sum Output: ${OUTPUTNAME}.sum\n";
...
if ($OUTPUT_TYPES{"sum_nbest"}) {
    ...
    print "Sum (nbest) Output: ${OUTPUTNAME}.sum_nbest\n";
}
...
foreach my $prefixed_id (sort keys %HYPS) {
	...
	if (! defined($REFS{$id}));
    # JGK: added dup line below for better error logging
    (print STDERR "Error: No matching reference for $rep_id.\n" and next) 
	if (! defined($REFS{$id}));
